...
To ensure confluence can run on OpenShift, the following changes were made:

* changed `RUN_USER daemon` to ID `1001`

* changed `RUN_GROUP daemon` to `root`

* added instruction to ensure write permissions for group `&& chmod -R g+w ${CONFLUENCE_INSTALL_DIR}/ \`

* added `USER 1001`

* moved `USER` and `WORKDIR` settings to the bottom of the Dockerfile

Please keep in mind this [timing issue](https://community.atlassian.com/t5/Confluence-questions/Using-a-Docker-instance-of-Confluence-Installation-Fails-When/qaq-p/731543)
